...
class WizardStateManager {
    ...
    constructor(moduleSystemData, flawData, destinyData, abilityData) {
      ...
      this.state = {
        ...
        abilities: [], // Array of {id, selections: [], tier: number}
        attributes: {},
        info: { name: '', bio: '' }
      };
    ...
    /**
     * Adds or updates an ability in the state.
     * Ensures only one ability per tier is selected.
     * @param {Object} newAbility - The ability object to add/update {id, tier, selections}.
     */
    addOrUpdateAbility(newAbility) {
      console.log(`WizardStateManager: Adding/updating ability: ${newAbility.id} for Tier ${newAbility.tier}`);
      // Filter out any existing ability from the same tier
      this.state.abilities = this.state.abilities.filter(ability =>
        ability.tier !== newAbility.tier
      );
      // Add the new ability

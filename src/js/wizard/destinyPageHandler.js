...
class DestinyPageHandler {
    ...
    _renderAbilitiesSection() {
      ...
      Object.entries(abilitiesByTier).forEach(([tier, abilityIds]) => {
        ...
        abilityIds.forEach(abilityId => {
          ...
          const isSelected = currentAbilitiesState.some(a => a.id === abilityId && a.tier === parseInt(tier));
    ...
    _handleDestinyOptionClick(e) {
      ...
      if (currentDestiny !== selectedDestinyId) {
        ...
        this.stateManager.set('abilities', []); // Reset abilities on destiny change
    ...
    _handleTierSelection(tier, abilityId) {
      ...
      // Create a new ability state object
    ...
    _handleAbilityOptionSelection(abilityId, optionId, isSelected, inputElement) {
      ...
      const abilityState = currentState.abilities.find(a => a.id === abilityId);
  
      if (!abilityState) {
        console.warn(`DestinyPageHandler._handleAbilityOptionSelection: Parent ability '${abilityId}' not found in state.`);
    ...
    _restoreState() {
      ...
      currentState.abilities.forEach(abilityState => {
        ...
        if (abilityCard) {
          ...
          console.log(`DestinyPageHandler._restoreState: Ability card "${abilityState.id}" (Tier ${abilityState.tier}) re-selected.`);
        }
      });
    ...
    _autoSelectSingleAbilityTier() {
      ...
      Object.entries(abilitiesByTier).forEach(([tier, abilityIds]) => {
        if (abilityIds.length === 1) {
          ...
          const isAlreadySelected = currentState.abilities.some(a => a.id === singleAbilityId && a.tier === parseInt(tier));
  
          if (!isAlreadySelected) {
            console.log(`DestinyPageHandler: Auto-selecting ability "${singleAbilityId}" for Tier ${tier}.`);
            ...
            if (!isParentAbilityCurrentlySelected) {
              ...
              if (optionInput.checked) { // Ensure unselected parent's options are unchecked
                ...
                if (abilityState) {
                  ...
                  this.stateManager.updateAbilitySelections(abilityId, abilityState.selections);
                }
              }
            } else {
